...
custom_modal_html = """
    ...
    <script type="text/javascript">
        ...
        function changeLanguage(langCode) {
            // Force translation via cookies robustly
...
new_nav_cta = """<div class="nav-cta" style="display: flex; align-items: center; gap: 10px;">
                <button onclick="openLangModal()" class="btn btn-ghost" style="padding: 8px; border-radius: 50%; display: flex; align-items: center; justify-content: center; width: 40px; height: 40px; border: 1px solid var(--border-subtle);" aria-label="Change Language">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-globe"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>
                </button>
...
for filepath in html_files:
    ...
    # 1. Replace nav-cta with new one featuring vector icon
    content = re.sub(r'<div class="nav-cta"[^>]*>[\s\S]*?<a href="https://dash\.netpin\.io/login"', new_nav_cta.split('<a href="https://dash.netpin.io/login"')[0] + '<a href="https://dash.netpin.io/login"', content)

    # 2. Inject modal at bottom
    if 'id="langModal"' not in content:
        content = re.sub(r'(<!-- Google Translate Script -->)', custom_modal_html + r'\n\1', content)

    with open(filepath, 'w') as f:
        f.write(content)
    print(f"Updated translation UI in {filepath}")
